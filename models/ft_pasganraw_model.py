...
# from .networks import PasNetPlusRaw, get_norm_layer, init_net
#
...
#                 ks_row_mse = F.mse_loss(ks_input, ks_gt, reduce=False).sum(1,keepdim=True).sum(2,keepdim=True).squeeze(1).squeeze(1) / (2*w)
...
#         self.register_buffer('seperate_mask', torch.FloatTensor(1, imSize, 1, 640, imSize)) #TODO:remove hardcoded dimension
#         self.seperate_mask.fill_(0)
#         for i in range(imSize):
...
#         masked_kspace = masked_kspace.view(bz*w, 2, h, w)
#         seperate_imgs = self.IFFT(masked_kspace).norm(dim=1).view(bz, w, h, w) # discard the imaginary part [B,imsize,imsize, imsize]
#
...
#
#         parser.add_argument('--n_recursive', type=int, default=3, help='number of recursive blocks in Reconstruction network')
...
#
#         # parser.set_defaults(which_model_netG='pasnetplus_rawdata')
#         parser.set_defaults(n_blocks=9)
#         # parser.set_defaults(n_recurive=3)
#         parser.set_defaults(mask_embed_dim=6)
#         parser.set_defaults(n_downsampling=3)
#
...
#         parser.set_defaults(fineSize=[640, 368])
...
#         parser.set_defaults(eval_full_valid=False)
...
#         self.netG = init_net(PasNetPlusRaw(opt.input_nc, opt.output_nc, opt.ngf, norm_layer=get_norm_layer(opt.norm),
#                         use_dropout=not opt.no_dropout, n_blocks=opt.n_blocks, n_recurive=opt.n_recursive, mask_embed_dim=opt.mask_embed_dim, n_downsampling=opt.n_downsampling, img_width=opt.fineSize[1],
#                         mask_cond=opt.mask_cond, use_deconv=opt.use_deconv), gpu_ids=self.gpu_ids)
#         self.netP = AUTOPARAM(length=self.num_stage, fixed_ws=opt.use_fixed_weight).to(self.device)
#         self.low_freq_count = 14
#         self.mask = create_mask(opt.fineSize[1], mask_fraction=self.opt.kspace_keep_ratio, mask_low_freqs=self.low_freq_count).to(self.device)
...
#                 pre_process = kspaceMap(imSize=opt.fineSize[1], no_embed=opt.no_kspacemap_embed).to(self.device)
#                 d_in_nc = opt.fineSize[1] + (6 if self.opt.mask_cond else 0)
...
#             self.imag_gt = torch.cuda.FloatTensor(opt.batchSize, 1, opt.fineSize[0], opt.fineSize[1])
#
#         self.zscore = 10
#
...
#
#     def calculate_loss(self, reconst_img, gt_img):
#         mse = float(F.mse_loss(reconst_img[:,:2,...].norm(dim=1,keepdim=True), gt_img[:,:2,...].norm(dim=1,keepdim=True), size_average=True))
#         ssim = float(util.ssim_metric(reconst_img[:, :2, ...].norm(dim=1, keepdim=True), gt_img[:, :2, ...].norm(dim=1, keepdim=True)))
#
#         return mse, ssim
#
...
#         # o = 2
#         # gaussian nll loss
#         l2 = self.criterion(fake_B[:,:,:,:], real_B[:,:,:,:])     #l2 = self.criterion(fake_B[:,:o,:,:], real_B[:,:o,:,:])
...
#                 mask = create_mask((batchSize, self.opt.fineSize[1]), mask_low_freqs=self.low_freq_count, random_frac=True,
#                                    mask_fraction=self.opt.kspace_keep_ratio).to(self.device)
#             elif self.opt.dynamic_mask_type == 'random_full':
#                 mask = create_mask((batchSize, self.opt.fineSize[1]), mask_low_freqs=self.low_freq_count, random_frac=True,
#                                    mask_fraction=self.opt.kspace_keep_ratio, random_full=True).to(self.device)
#             elif self.opt.dynamic_mask_type == 'random_lines':
#                 seed = np.random.randint(10000)
#                 mask = create_mask((batchSize, self.opt.fineSize[1]), mask_low_freqs=self.low_freq_count, random_frac=False,
#                                    mask_fraction=self.opt.kspace_keep_ratio, seed=seed).to(self.device)
#         else:
#             mask = create_mask((batchSize, self.opt.fineSize[1]), mask_low_freqs=self.low_freq_count, random_frac=False,
#                                mask_fraction=self.opt.kspace_keep_ratio).to(self.device)
#
...
#         mask, image, kspace = input
#         image = image.to(self.device)
#         kspace = kspace.to(self.device)
#
#         # do clip internally in loader
#         # image = self._clamp(image).detach()
#         # input = self._clamp(input)
#
#         if self.isTrain and self.opt.dynamic_mask_type != 'None' and not self.validation_phase:
#             self.mask = self.gen_random_mask(batchSize=image.shape[0])
#         else:
#             # use provided fixed masked
#             self.mask = mask.to(self.device)
#
#         # ifft_image = self.IFFT(kspace.permute(0,3,1,2) * self.mask)
#         # ifft_image = ifftshift(ifft_image, dim=(1,2,3))
#         #
#         # # kspace_shifted = ifftshift(kspace.permute(0, 3, 1, 2), dim=(1, 2, 3))
#         # # ifft_image = self.IFFT(kspace_shifted * self.mask)
#         # # ifft_image = self._clamp(ifft_image).detach()
#
#         self.real_A = self.IFFT(self.FFT(image.permute(0,3,1,2)) * self.mask) #ifft_image
#         self.real_B = image.permute(0,3,1,2)
#
#         # import matplotlib
#         # matplotlib.use('agg')
#         # import matplotlib.pyplot as plt
#         #
#         # k_A = self.FFT(self.real_A)[0].squeeze(0)
#         # k_A = torch.norm(k_A, p=2, dim=0)
#         # plt.imsave(os.path.join(self.save_dir, 'k_A.png'), np.log(k_A.detach().cpu().numpy()), cmap='gray')
#         #
#         # k_B = self.FFT(self.real_B)[0].squeeze(0)
#         # k_B = torch.norm(k_B, p=2, dim=0)
#         # plt.imsave(os.path.join(self.save_dir, 'k_B.png'), np.log(k_B.detach().cpu().numpy()), cmap='gray')
#         #
#         # masked_k_B = self.FFT(self.real_B * self.mask)[0].squeeze(0)
#         # masked_k_B = torch.norm(masked_k_B, p=2, dim=0)
#         # plt.imsave(os.path.join(self.save_dir, 'masked_k_B.png'), np.log(masked_k_B.detach().cpu().numpy()), cmap='gray')
#
#         # debug : these should be 0
#         # print((self.FFT(self.real_B) * self.mask - self.FFT(self.real_A)).sum())
#         # print(((self.FFT(self.real_B) * self.mask - self.FFT(self.real_A)) ** 2).mean())
#
#     def forward(self, sampling=False):
#         # conditioned on mask
#         h, b = self.mask.shape[3], self.real_A.shape[0]
...
#
#         self.loss_MSE, self.loss_SSIM = self.calculate_loss(self.fake_B, self.real_B)
#
...
#
#
#     def validation(self, val_data_loader, how_many_to_display=16, how_many_to_valid=4096*4, n_samples=8, metasavepath=None):
#
#         if self.mri_data:
...
#                 # Issue : Only works if batch_size >= sqrt(how_many_to_display). Why concatenate logvars of each encoder block?
#                 # for s in range(2):
...
#                 log_var_vis = self.logvars[-1].cpu()
...
#             pickle_file['mask'] = self.mask.repeat(1, 1, 640, 1).cpu().numpy()
#             # pickle_file['mask'] = self.mask.repeat(1,1,1,368).cpu().numpy()
...
#         # ## test plots
#         # # Plot real_B
#         # x = torch.norm(self.real_B, p=2, dim=1, keepdim=True)[0].data
#         # image_pil = Image.fromarray(x.squeeze(0).cpu().float().numpy())
#         #
#         # if image_pil.mode != 'RGB':
#         #     image_pil = image_pil.convert('RGB')
#         # image_pil.save(os.path.join(self.opt.checkpoints_dir, self.opt.name, 'real_B.png'))
#         # visuals['real_B'] = tensor2im(x)
#         #
#         # # Plot fake_B
#         # x = torch.norm(self.fake_B, p=2, dim=1, keepdim=True)[0].data
#         # image_pil = Image.fromarray(x.squeeze(0).cpu().float().numpy())
#         #
#         # if image_pil.mode != 'RGB':
#         #     image_pil = image_pil.convert('RGB')
#         # image_pil.save(os.path.join(self.opt.checkpoints_dir, self.opt.name, 'fake_B.png'))
#         # visuals['fake_B'] = tensor2im(x)
#         #
#         # # Plot k_real_B
#         # x = self.FFT(self.real_B)
#         # x = torch.norm(x, p=2, dim=1, keepdim=True)[0].data
#         # image_pil = Image.fromarray(np.log(x.squeeze(0).cpu().float().numpy()))
#         #
#         # if image_pil.mode != 'RGB':
#         #     image_pil = image_pil.convert('RGB')
#         # image_pil.save(os.path.join(self.opt.checkpoints_dir, self.opt.name, 'k_real_B.png'))
#         # visuals['k_real_B'] = tensor2im(x)
#         #
#         # # Plot k_fake_B
#         # x = self.FFT(self.fake_B)
#         # x = torch.norm(x, p=2, dim=1, keepdim=True)[0].data
#         # image_pil = Image.fromarray(np.log(x.squeeze(0).cpu().float().numpy()))
#         #
#         # if image_pil.mode != 'RGB':
#         #     image_pil = image_pil.convert('RGB')
#         # image_pil.save(os.path.join(self.opt.checkpoints_dir, self.opt.name, 'k_fake_B.png'))
#         # visuals['k_fake_B'] = tensor2im(x)
#         #
#         #
#         # ## end of test plots
#
...
#             mse, ssim = self.calculate_loss(self.fake_B, self.real_B)
...
#             sys.stdout.write('\r validation (%d/%d) [MSE: %.5f SSIM: %.5f]' % (
#             val_count, how_many_to_valid, np.mean(losses['MSE']), np.mean(losses['SSIM'])))
#             sys.stdout.flush()
#
