...
if __name__ == '__main__':
    ...
    for i, data in enumerate(test_data_loader):
        ...
        for s in range(n_forward):
            ...
            # calcuate which line to replace and replace it
            new_fakeB, new_mask = replace_kspace_lines(old_fakeB, realB, old_mask, n_recom_each, None, cand_line)

            old_fakeB = new_fakeB
            old_mask = new_mask
            score_list_random.append(compute_scores(old_fakeB, realB))
        ...
        if use_forward:
            ...
            for s in range(n_forward):
                ...
                old_fakeB = model.fake_B
                old_mask = new_mask
                score_list_random2.append(compute_scores(old_fakeB, realB))
                
        score = OrderedDict()
        score['random'] = np.array(score_list_random)
        if use_forward:
            score['random_forward'] = np.array(score_list_random2)

        score['algorithm'] = np.array(score_list)

        visuals['recommend_kspace_lines'] = draw_curve(score)
        visuals['kspace D-score'] = draw_curve2(np.array(score_D))
