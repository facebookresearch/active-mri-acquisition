...
def draw_curve3(uncertainty):
    ...
    for i, (key, values) in enumerate(uncertainty.items()):
        ...
        xval = [float(a)/len(data) * Percentage[-1] for a in range(0, len(data))]
...
def draw_curve2(data):
    ...
    xval = [float(a)/data.shape[0] * Percentage[-1] for a in range(0, data.shape[0])]
...
def draw_curve(data):
    ...
    for i, (key, values) in enumerate(data.items()):
        ...
        ls = '-' if 'algorithm' in key else "--"
        key_name = key.replace('_', '+')
        xval = [float(a)/values.shape[0] * Percentage[-1] for a in range(0, values.shape[0])]
...
def animate(images, masks, eval_scores, uncertainty, D_score, xval,
    ...
    ani.save(save_path)
    # ani.save(opt.name+"_measure_acq_animatio.gif", writer='imagemagick', fps=10)
...
if __name__ == '__main__':
    ...
    if n_forward is None:
        n_forward = unobsered_line_n // 2 if conjudge_symmetric else unobsered_line_n // n_recom_each
    imsize = opt.fineSize
    np.random.seed(1234)

    print(f'>> start from {observed_line_n} lines and add {n_recom_each} line per forward, need {n_forward} forward')
    for i, data in enumerate(test_data_loader):
        print('Conduct iteration', i)
        ...
        score_list['algorithm_errormap'] = [compute_errormap(fakeB, realB)]
        ...
        for nf in range(n_forward):
            ...
            score_list['algorithm_errormap'].append(compute_errormap(old_fakeB, realB))
        ...
        video_path = animate(score_list['algorithm_errormap'], score_list['algorithm_mask'], score_list['algorithm'], uncertainty_list['algorithm'], 
                        score_D, start_line=observed_line_n, xval=Percentage)
        ...
        ## draw curves 
        visuals['recommend_kspace_lines'] = draw_curve(score)
        visuals['kspace D-score'] = draw_curve2(score_D)
        visuals['model_uncertainty'] = draw_curve3(score_uncetainty)
        ...
        if save_metadata:
            ...
            MetaData['data'] = data
            with open(os.path.join(save_dir, 'metadata.pickle'),'wb') as f:
                pickle.dump(MetaData, f)
                print('save at', os.path.join(save_dir, 'metadata.pickle'))

            visuals['metadata'] = 'metadata.pickle'

        # draw percentile mse vs uncertainty
