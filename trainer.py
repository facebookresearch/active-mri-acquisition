...
class Trainer:
    ...
    def inference(self, batch):
        ...
        with torch.no_grad():
            ...
            if self.evaluator is not None:
                self.evaluator.eval()
                reconstructor_eval = self.evaluator(reconstructed_image, mask_embedding, mask)
    ...
    def update(self, batch):
        ...
        if self.evaluator is not None:
            ...
            output = self.evaluator(detached_fake, mask_embedding, mask if self.options.add_mask_eval else None)
            loss_D_fake = self.losses['GAN'](
                output, False, mask, degree=0, pred_and_gt=(detached_fake, target))

            real = target
            output = self.evaluator(real, mask_embedding, mask if self.options.add_mask_eval else None)
            ...
            if not self.options.only_evaluator:
                output = self.evaluator(fake, mask_embedding, mask if self.options.add_mask_eval else None)
