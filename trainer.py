...
class Trainer:
    ...
    def update(self, batch):
        ...
        if self.evaluator is not None:
            ...
            output = self.evaluator(detached_fake, mask_embedding, raw=(self.options.dataroot=='KNEE_RAW'))
            loss_D_fake = self.losses['GAN'](
                output, False, mask, degree=0, pred_and_gt=(detached_fake, target))

            real = target
            output = self.evaluator(real, mask_embedding, raw=(self.options.dataroot=='KNEE_RAW'))
            ...
            if not self.options.only_evaluator:
                output = self.evaluator(fake, mask_embedding, raw=(self.options.dataroot=='KNEE_RAW'))
