...
for mask in basic_rnl; do
    ...
    --mask_type ${mask} \
    --name ${mask}_data_aug_no_mask_emb_beta=10_post_refactor_2 --number_of_evaluator_convolution_layers 4 \
    --checkpoints_dir ${CHECKPOINTS_DIR} --mask_embed_dim 0 --add_mask_eval \
    --use_submitit \
    --submitit_logs_dir ${CHECKPOINTS_DIR}/submitit_logs \
    --batchSize 2 \
    --gpu_ids 0,1,2,3,4,5,6,7 --print_freq 50 --lr 0.0006 --grad_ctx --max_epochs 300 \
