...
class EvaluatorBasedValueNetwork(nn.Module):
    ...
    def __init__(
        ...
        num_actions = image_width - 2 * initial_num_lines_per_side
        self.evaluator = m20_evaluator.EvaluatorNetwork(
    ...
    def forward(self, x):
        ...
        filtered_mask = x[
            :, 0, -2, self.initial_num_lines_per_side : -self.initial_num_lines_per_side
        ]
    ...
    def _get_action_no_replacement(
        ...
        if self.opts.obs_type == "only_mask":
            previous_actions = np.nonzero(
                observation[
                    self.opts.initial_num_lines_per_side : -self.opts.initial_num_lines_per_side
                ]
            )[0]
        else:
        ...
        if sample < eps_threshold:
            return random.choice(
                [x for x in range(self.num_actions) if x not in previous_actions]
            )
        with torch.no_grad():
            ...
            with torch.no_grad():
                ...
                if not_done_mask.any().item() != 0:
                    ...
                    target_values[not_done_mask] = (
                        target_net.forward(next_observations)["qvalue"]
...
class DQNTester:
    def __init__(self, training_dir):
        ...
        self.latest_policy_path = DQNTrainer.get_name_latest_checkpoint(
            self.training_dir
        )
    ...
    def init_all(self):
        ...
        # Initialize policy
        self.policy = DDQN(
            self.env.action_space.n, rl_env.device, None, self.options
        ).to(rl_env.device)
    ...
    def __call__(self):
        ...
        while not training_done:
            ...
            test_score, _ = evaluation.test_policy(
                self.env,
                self.policy,
                self.writer,
                self.logger,
...
class DQNTrainer:
    ...
    def load_checkpoint_if_needed(self):
        if self.options.dqn_only_test:
            ...
            if os.path.isfile(policy_path):
                self.load(policy_path)
                self.logger.info(
                    f"Loaded DQN policy found at {policy_path}. Steps was set to "
                    f"{self.steps}. Episodes set to {self.episode}."
                )
                self.env._image_idx_train = self.episode % self.env.num_train_images
            else:
                self.logger.info(
                    f"No policy found at {policy_path}, continue without checkpoint."
                )
            if self.load_replay_mem:
                ...
                if os.path.isfile(memory_path):
                    ...
                    self.logger.info(
                        f"Loaded replay memory from {memory_path}. "
                    ...
                    self.logger.warning(
                        f"load_replay_mem was True, but no checkpoint was found "
    ...
    def _train_dqn_policy(self):
        ...
        while self.steps < self.options.num_train_steps:
            ...
            auc_score = self.env.compute_score(True, use_current_score=True)[0][
            ...
            while not done:
                ...
                if self.options.obs_type == "only_mask":
                    is_repeated_action = bool(
                        obs[action + self.options.initial_num_lines_per_side]
                    )
                else:
                ...
                if self.steps % 250 == 0:
                    ...
                    if update_results is not None:
                        ...
                        if update_results["value"] is not None:
                            self.writer.add_scalar(
                                "value", update_results["value"], self.steps
                            )
                            self.writer.add_scalar(
                                "advantage", update_results["advantage"], self.steps
                            )
